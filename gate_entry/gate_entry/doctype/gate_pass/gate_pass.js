...
frappe.ui.form.on("Gate Pass", {
	...
	refresh(frm) {
		...
		if (frm.is_new() && !frm.doc.security_guard_name) {
			frm.set_value('security_guard_name', frappe.session.user_fullname || frappe.session.user);
		}
...
function create_purchase_receipt(frm) {
	frappe.confirm(
		__('Create Purchase Receipt from this Gate Pass?'),
		function() {
			frappe.call({
...
function create_subcontracting_receipt(frm) {
	frappe.confirm(
		__('Create Subcontracting Receipt from this Gate Pass?'),
		function() {
			frappe.call({
				...
				callback: function(r) {
					if (r.message) {
						frappe.show_alert({
							message: __('Subcontracting Receipt {0} created successfully', [r.message]),
							indicator: 'green'
						});
